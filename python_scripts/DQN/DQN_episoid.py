...
from python_scripts.Log_write import Log_write
...
def DQN_episoid(model_path=None):
    ...
    # 查找现有的日志文件，确定最新的编号
    # 抓取阶段：
    log_pattern = os.path.join(path_list['catch_log_path'], 'catch_log_*.json')
    ...
    if existing_logs:
        ...
        # 没有现有日志文件，从1开始
        new_log_num = 1
    log_file_latest_catch = os.path.join(path_list['catch_log_path'], f"catch_log_{new_log_num}.json")
    print(f"将使用新的抓取日志目录: {log_file_latest_catch}")

    # 抬腿阶段：
    log_pattern = os.path.join(path_list['tai_log_path'], 'tai_log_*.json')
    existing_logs = glob.glob(log_pattern)
    latest_num = 0
    if existing_logs:
        # 从文件名中提取编号
        ...
        # 没有现有日志文件，从1开始
        new_log_num = 1
    log_file_latest_tai = os.path.join(path_list['tai_log_path'], f"tai_log_{new_log_num}.json")
    ...
    if model_path:  # 如果指定了模型路径
        ...
        # 获取所有模型文件
        model_files = glob.glob(path_list['model_path_catch'] + '/dqn_model_*.ckpt')
        if model_files:
    ...
    # 抬腿模型加载
    model_files_tai = glob.glob(path_list['model_path_tai'] + '/dqn_model_tai_*.ckpt')
    if model_files_tai:
        try:
    ...
    for i in range(episode_start, episode_start + 50000):  # 从episode_start开始，最多再训练50000个周期
        ...
        while True:
            ...
            if len(rpm) > 5000 and done == 1:  # 只有在buffer中存满了数据才会学习
                if goal == 1:  # 如果达到目标
                    print("goal = 1")
                    save_path = path_list['model_path_catch'] + '/dqn_model_%s.ckpt' % i  # 保存模型
                    torch.save(dqn.eval_net, save_path)
                loss = dqn.learn(rpm)  # 学习
                log_writer_catch.add(loss=loss)
                # 每500步保存一次模型
                if i % 500 == 0:
                    path = path_list['model_path_catch'] + '/dqn_model_%s.ckpt' % i
